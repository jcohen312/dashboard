...
app.layout = html.Div([
    html.Div([
        ...
        html.Div(children="Revenue Decomposition",
                 style={'font': style['font'],
    ...
    html.Div([
        html.Div(children="Industry Decomposition",
                 style={'font': style['font'],
                        'color': style['background'],
                        'backgroundColor': style['button'],
                        'fontSize': 22,
        ...
        dcc.Dropdown(id='industry dropdown',
                     options=[{'label': sport, 'value': sport} for sport in combined.sport_type.unique()],
                     multi=True,
                     style={'width': '65%'}
                     ),

        dcc.Graph(id = 'industry graph 1',
                  style={'width': '65%'}
        ),
...
def plot_org(orgs=[], transaction_type='Net Revenue'):
    ...
    layout = go.Layout(title=', '.join(orgs) + ' {}'.format(transaction_type),
                       xaxis={'tickangle': 45,
                              'tickvals': grouped[orgs[0]]['Revenue'].subtract(grouped[orgs[0]]['Credit'],
                                                                               fill_value=0).index})
